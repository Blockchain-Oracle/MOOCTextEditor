 ...
 * It does one pass through the document to count the number of syllables,
 * words,
...
public class EfficientDocument extends Document {
	...
	private void processText() {
		...
		for (int i = 0; i < tokens.size(); i++) {
			...
			if (isWord(token)) {
				...
				// Check if it's the last token to count it as a sentence
				if (i == tokens.size() - 1) {
					numSentences++;
				}
			} else {
	 ...
	 * To calculate the the number of syllables in a word, it uses the following
	 * rules:
